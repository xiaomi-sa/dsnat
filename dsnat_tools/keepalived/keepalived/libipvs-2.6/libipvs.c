...
static int ipvs_nl_fill_zone_attr(struct nl_msg *msg, ipvs_zone_t *zone)
{
	...
	nl_service = nla_nest_start(msg, IPVS_CMD_ATTR_ZONE);
	if (!nl_service)
		return -1;

	NLA_PUT(msg, IPVS_LADDR_ATTR_ADDR , sizeof(zone->addr), &(zone->addr));
	NLA_PUT(msg, IPVS_LADDR_ATTR_ADDR , sizeof(zone->netmask), &(zone->netmask));
...
static int ipvs_zones_parse_cb(struct nl_msg *msg, void *arg)
{
	...
	struct ip_vs_get_services **getp = (struct ip_vs_get_zones **)arg;
	struct ip_vs_get_services *get = (struct ip_vs_get_zones *)*getp;
	...
	memcpy(&(get->entrytable[i].netmask), nla_data(zone_attrs[IPVS_ZONE_ATTR_NETMASK]),
	       sizeof(get->entrytable[i].addr));
	i++;
...
ipvs_zone_entry_t *
ipvs_get_zone(__be32 addr, __be32 netmask)
{
...
struct ip_vs_get_laddrs *ipvs_get_laddrs(ipvs_zone_entry_t *zone)
{
	...
	if (try_nl) {
		...
		NLA_PUT(msg, IPVS_SVC_ATTR_ADDR, sizeof(zone->addr),
								&zone->addr);
		NLA_PUT(msg, IPVS_SVC_ATTR_ADDR,  sizeof(zone->netmask),
								&zone->netmask);
