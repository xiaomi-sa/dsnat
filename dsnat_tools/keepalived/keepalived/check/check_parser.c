...
static void
zone_handler(vector strvec)
{
	alloc_zone(VECTOR_SLOT(strvec, 1), VECTOR_SLOT(strvec, 2));
}
...
{
	zone *zone = LIST_TAIL_DATA(check_data->zone);
	
	zone->local_addr_gname = set_value(strvec);
}
static void 
...
check_init_keywords(void)
{
	...
	/* zone mapping */
	install_keyword_root("zone", &zone_handler);
