	...
	if (status == connect_success) {
		...
		if (svr_checker_up(DOWN, checker->id, checker->rs)) {
			if(status == connect_timeout){
				log_message(LOG_INFO, "TCP connection timeout to [%s:%d] !!!"
						    , inet_sockaddrtos(&tcp_check->dst)
						    , ntohs(inet_sockaddrport(&tcp_check->dst)));				
			}else if(status == connect_error){
				log_message(LOG_INFO, "TCP connection failed to [%s:%d]  !!!"
						    , inet_sockaddrtos(&tcp_check->dst)
						    , ntohs(inet_sockaddrport(&tcp_check->dst)));			
			}else if(status == connect_in_progress){
				log_message(LOG_INFO, "TCP connection to [%s:%d] still IN_PROGRESS. !!!"
						    , inet_sockaddrtos(&tcp_check->dst)
						    , ntohs(inet_sockaddrport(&tcp_check->dst)));			
			}			
